...
test_that("from_POSIXct works", {
  ...
  # -- detect OS
  # there is a problem on Linux where timezone uses region/country
  tz <- if(Sys.info()[['sysname']] == "Linux") "Europe/Paris" else "CET"

  # -- convert POSIXct to ISO-8601
  x <- from_POSIXct(data.frame(a = as.POSIXct("2025-05-13 12:15:30", tz = tz)))
